---
name: codebase-to-course-trae
description: "Turn any codebase into a beautiful, interactive single-page HTML course that teaches how the code works to non-technical people. Trae/Windows adapted version. Use this skill whenever someone wants to create an interactive course, tutorial, or educational walkthrough from a codebase or project. Also trigger when users mention 'turn this into a course,' 'explain this codebase interactively,' 'teach this code,' 'interactive tutorial from code,' 'codebase walkthrough,' 'learn from this codebase,' or 'make a course from this project.' This skill produces a stunning, self-contained HTML file with scroll-based navigation, animated visualizations, embedded quizzes, and code-with-plain-English side-by-side translations."
---
...
If the user provides a GitHub link, clone the repo first before starting the analysis. Use a platform-appropriate temporary directory:
- On macOS/Linux: `git clone <url> /tmp/<repo-name>`
- On Windows/Trae: clone into a writable temp or workspace directory such as `%TEMP%\<repo-name>` or `<workspace>\tmp\<repo-name>`
...
course-name/
  ...
  modules/
    ...
    02-actors.html
    ...
  build.ps1        ← Windows/Trae assembly helper
  index.html       ← assembled by build.sh or build.ps1 (do not write manually)
```

**Step 1 (both paths): Setup** — Create the course directory. Copy these files verbatim using Read + Write (do not regenerate their contents):
...
- `references/build.ps1` → `course-name/build.ps1`
...
**Step 4 (both paths): Assemble** — Run the platform-appropriate build script from the course directory:
```bash
cd course-name && bash build.sh
```
On Windows/Trae, prefer:
```powershell
Set-Location course-name
.\build.ps1
```
...
After running the appropriate build script (`build.sh` or `build.ps1`), open `index.html` in the browser. Walk the user through what was built and ask for feedback on content, design, and interactivity.

### Batch Delivery Notes (Multi-Repo Runs)
...
For every generated course, assign a source confidence level and surface it in delivery notes:

- **A (Code-level)**: built from cloned repository files (entrypoints, modules, configs).
- **B (Repo-doc level)**: built mainly from `README` and repository docs when full clone is unavailable.
- **C (Web level)**: built from official product pages or external docs when repository content is inaccessible.

If the run is not level A, explicitly list what evidence is missing and which modules may be less precise.

### Network Fallback Contract

When clone/network is unstable, do not block the whole batch:

1. Try shallow clone (`--depth 1 --single-branch`) with retries.
2. Fallback to blobless clone (`--filter=blob:none`) with retries.
3. Fallback to repository web docs (`README`, docs pages) and continue generation as B/C level.
4. Keep a per-repo status summary (`cloned`, `fallback-readme`, `fallback-web`) in final output.

### Quality Gate Contract

A course is considered **final** only if each module includes all of the following:

- at least one source-backed code/config snippet
- one plain-English translation block
- one interactive quiz
- glossary tooltip(s) on first technical terms

A course can be marked **draft** only when evidence is incomplete or one of the required elements is missing.

### Header Link Contract

If the user asks for a header icon link:

- default target is the original source repository URL
- custom target is allowed only if the user explicitly specifies it
- include the final resolved URL in delivery notes for verification

### Batch Resume Contract

For long multi-repo runs, keep resumable execution behavior:
